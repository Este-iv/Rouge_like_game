...
{
  if (charpair(next) &&
      ((next[dim_y] != get_x(d->pc)) ||
       (next[dim_x] != get_y(d->pc)))) {
    do_combat(d, c, charpair(next));
  } else {
    /* No character in new position. */

    d->character[get_y(d->pc)][get_x(d->pc)] = NULL;
    set_y_pos(c, next[dim_y]);
    set_x_pos(c, next[dim_x]);
    d->character[get_y(d->pc)][get_x(d->pc)] = c;
  }
}
...
;
  ...
  while (pc_is_alive(d) &&
    ...
    if (!get_life(d->pc)) {
      if (d->character[get_y(d->pc)][get_x(d->pc)] == c) {
        d->character[get_y(d->pc)][get_x(d->pc)] = NULL;
      }
      if (c != &d->pc) {
        event_delete(e);
...
void dir_nearest_wall(dungeon_t *d, character_t *c, pair_t dir)
{
  ...
  if (get_x(d->pc) != 1 && get_x(d->pc) != DUNGEON_X - 2) {
    dir[dim_x] = (get_x(d->pc) > DUNGEON_X - get_x(d->pc) ? 1 : -1);
  }
  if (get_y(d->pc) != 1 && get_y(d->pc) != DUNGEON_Y - 2) {
    dir[dim_y] = (get_y(d->pc) > DUNGEON_Y - get_y(d->pc) ? 1 : -1);
  }
}

uint32_t against_wall(dungeon_t *d, character_t *c)
{
  return (mapxy(get_x(c)- 1,
...
uint32_t move_pc(dungeon_t *d, uint32_t dir)
{
  ...
  case 9:
    ...
    break;
  case '<':
    if (mappair(char_get_pos(d->pc)) == ter_stairs_up) {
      was_stairs = 1;
      new_dungeon_level(d, '<');
    }
    break;
  case '>':
    if (mappair(char_get_pos(d->pc)) == ter_stairs_down) {
      was_stairs = 1;
      new_dungeon_level(d, '>');
    }
    break;
