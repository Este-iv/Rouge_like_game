...
{
  d->io_offset[dim_x] = ((get_x(d->pc) - 10) / 20) * 20;
  ...
  d->io_offset[dim_y] = ((get_y(d->pc) - 4) / 7) * 7;
...
void io_update_offset(dungeon_t *d)
{
  ...
  x = (40 + d->io_offset[dim_x]) - get_x(d->pc);
  y = (11 + d->io_offset[dim_y]) - get_x(d->pc);
  ...
  for (y = 0; y < 21; y++) {
    for (x = 0; x < 80; x++) {
      ...
      if (d->character[d->io_offset[dim_y] + y]
                      [d->io_offset[dim_x] + x]) {
        mvaddch(y + 1, x, get_symbol(d->character[d->io_offset[dim_y] + y]
                                      [d->io_offset[dim_x] + x]));
      } else {
...
uint32_t io_teleport_pc(dungeon_t *d)
{
  ...
  set_y(d->pc, dest[dim_y]);
  set_x(d->pc,dest[dim_x]);
    ...
    snprintf(s[i], 40, "%16s%c: %2d %s by %2d %s",
             (get_symbol(c[i]) == 'd' ? "A tenacious " :
              adjectives[rand() % (sizeof (adjectives) /
                                   sizeof (adjectives[0]))]),
             get_symbol(c[i]),
             abs(get_y(c[i])- get_y(c[i])),
             ((get_y(c[i]) - get_y(c[i])) <= 0 ?
              "North" : "South"),
             abs(get_x(c[i]) - get_x(c[i])),
             ((get_x(c[i]) - get_x(c[i])) <= 0 ?
              "East" : "West"));
    if (count <= 13) {
...
static int compare_monster_distance(const void *v1, const void *v2)
{
  ...
  return (dungeon->pc_distance[get_y(*c1)][get_x(*c1)] -
          dungeon->pc_distance[get_y(*c2)][get_y(*c2)]);
}
