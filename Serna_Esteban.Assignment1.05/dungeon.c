...
//works!!!
void stair_maker(dungeon_t *d) {
    int i;
    d->num_stairs = 2;
    d->stairs = malloc(sizeof(*d->stairs) * d->num_stairs);
    for (i = 0; i < d->num_stairs; i++) {

        d->stairs[1].position[dim_y] = rand_range(d->rooms[1].position[dim_y],
                                                  (d->rooms[1].position[dim_y] +
                                                   d->rooms->size[dim_y] - 1));
        d->stairs[1].position[dim_x] = rand_range(d->rooms[1].position[dim_x],
                                                  (d->rooms[1].position[dim_x] +
                                                   d->rooms->size[dim_x] - 1));

        //                              rand_range(...) To test for rand.

        d->stairs[0].position[dim_y] = rand_range (d->rooms[2].position[dim_y],
                                                   (d->rooms[2].position[dim_y] +
                                                    d->rooms->size[dim_y] - 1));
        d->stairs[0].position[dim_x] = rand_range(d->rooms[2].position[dim_x],
                                                  (d->rooms[2].position[dim_x] +
                                                   d->rooms->size[dim_x] -1));
...
//                   down=0  up=1
        ...
        d->stairs[0].icon = rep[0];

        d->stairs[1].icon = rep[1];
    }
}
...
void render_dungeon(dungeon_t *d) {
    ...
    for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++) {
        for (p[dim_x] = 0; p[dim_x] < DUNGEON_X; p[dim_x]++){
            ...
            for (i = 0; i < d->num_stairs; i++) {
                if (p[dim_x] == d->stairs[i].position[dim_x] &&
                    p[dim_y] == d->stairs[i].position[dim_y]) {
                    putchar(d->stairs[i].icon);
...
void nrender(dungeon_t *d) {
    ...
    for (p[dim_y] = d->pc.position[dim_y] - 10; p[dim_y] < d->pc.position[dim_y] + 10; p[dim_y]++) {
        x = 0;
        for (p[dim_x] = d->pc.position[dim_x] - 40; p[dim_x] < d->pc.position[dim_x] + 40; p[dim_x]++) {
            f = 1;
            temp[dim_x] = p[dim_x];
            temp[dim_y] = p[dim_y];
            if (p[dim_x] < 0) {
            ...
            if (f == 1) {
                if (charpair(temp)) {
                    mvaddch(y, x, charpair(temp)->symbol);
                } else {
                    switch (mappair(temp)) {
                        case ter_wall:
                        case ter_wall_immutable:
                ...
                for (i = 0; i < d->num_stairs; i++) {
                    if (temp[dim_x] == d->stairs[i].position[dim_x] &&
                        temp[dim_y] == d->stairs[i].position[dim_y]) {
                        mvaddch(y, x, d->stairs[i].icon);
