...
// uint32_t get_color(object *c){ return c->color}

// follow this blueprint for the objects on the map.
void gen_monsters(dungeon_t *d)
{
  ...
  uint32_t c;
  //this is where the objects exist
//  uint32_t dice;
  // uint32_t hitpoints;
  // uint32_t attribute;
  dice damage;
  ...
  for (i = 0; i < d->num_monsters; i++) {
    ...
    int q;
    q = rand() % d->monster_descriptions.size();
    m->position[dim_y] = p[dim_y];
    m->position[dim_x] = p[dim_x];
    d->character_map[p[dim_y]][p[dim_x]] = m;
    // change to monster_description
    m->speed = d->monster_descriptions[q].speed.roll();
    m->alive = 1;
    m->sequence_number = ++d->character_sequence_number;
    //change to monster_description
    m->characteristics = rand() & 0x0000000f;
    m->color = d->monster_descriptions[q].color.at(0);
    m->abilities =d->monster_descriptions[q].abilities;
    m->discription = &d->monster_descriptions[q].description;
    m->name = &d->monster_descriptions[q].name;
    m->hitpoints =d->monster_descriptions[q].hitpoints.roll();
    m->damage = d->monster_descriptions[q].damage;
    // change to monster_description
    m->symbol = d->monster_descriptions[q].symbol;
    m->have_seen_pc = 0;
